...
class ICIFNet(nn.Module):
    def __init__(self,num_classes=2, drop_rate=0.2, normal_init=True, pretrained=False):
        ...
        resnet_weight = Path(__file__).resolve().parent / "pretrained" / "resnet18-5c106cde.pth"
        if pretrained and resnet_weight.exists():
            self.resnet.load_state_dict(torch.load(str(resnet_weight), map_location="cpu"))
        # self.resnet.fc = nn.Identity()
        self.resnet.layer4 = nn.Identity()

        self.backbone = pvt_v2_b1()  # [64, 128, 320, 512]
        path = Path(__file__).resolve().parent / "pretrained" / "pvt_v2_b1.pth"
        if pretrained and path.exists():
            save_model = torch.load(str(path), map_location="cpu")
